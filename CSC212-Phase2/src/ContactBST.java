...
public class ContactBST<T> {
	...
	private boolean phoneNumberExistsRec(BSTNode<Contact> current, String phoneNumber) {
		...
		return phoneNumberExistsRec(current.getLeft(), phoneNumber)
				|| phoneNumberExistsRec(current.getRight(), phoneNumber);
	}

	public Contact find(String name) {
		current = root;
		return findRec(current, name);
	}
	...
	public ContactBST<T> searchCriteria(String searchCriteria, String searchValue) {// value = name, phonenumber, email,
																					// address or birthday
	...
	private void searchCriteriaRec(BSTNode<Contact> current, String searchCriteria, String searchValue,
			ContactBST<T> resultTree) {
		if (current == null) {
			return;
		}
		...
		if (searchCriteria.equalsIgnoreCase("email")
				&& current.getData().getEmailAddress().equalsIgnoreCase(searchValue)) {
			resultTree.insert(current.getData());
		}
		if (searchCriteria.equalsIgnoreCase("address")
				&& current.getData().getAddress().equalsIgnoreCase(searchValue)) {
			resultTree.insert(current.getData());
		}
